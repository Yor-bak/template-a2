...
export interface Worker {
  ...
  createdAt: string;
  // Worker-portal login credential (set by the specialist). Plain text — MVP/demo only,
  // stored in localStorage. Login uses `email` + `password`.
  password?: string;
}

// ─── Delegated task ───────────────────────────────────────────────────────────

export type TaskStatus = "assigned" | "completed" | "incomplete";
...
export interface WorkerTask {
  ...
  status: TaskStatus;
  notes?: string;
  dueDate?: string;        // ISO date the task is scheduled for
  assignedAt: string;
  completedAt?: string;
  invoiceId?: string;      // set once the worker generates the invoice for this task
}

// ─── Invoice (receipt) generated by a worker for a completed task ──────────────

export interface WorkerInvoice {
  id: string;
  folio: string;           // human-readable folio, e.g. "REC-000123"
  taskId: string;
  workerId: string;
  workerName: string;
  clientName: string;
