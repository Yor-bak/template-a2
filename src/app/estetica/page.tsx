...
export default function EsteticaTemplate() {
  return (
    <div
      className={`${playfair.variable} ${sora.variable} min-h-screen bg-[#f7f3ec] text-[#1a1a1a]`}
      style={{ fontFamily: "var(--f-sora)" }}
    >
      {/* Floating round CTA, not a header/nav bar */}
      <a
        href={clinic.whatsapp}
        className="fixed bottom-6 right-6 z-30 flex h-16 w-16 items-center justify-center rounded-full bg-[#1a1a1a] text-center text-[0.6rem] font-semibold uppercase leading-tight text-[#f7f3ec] shadow-lg transition hover:bg-[#c9a468] hover:text-[#1a1a1a]"
      >
        Agendar<br />cita
      </a>
      ...
      <section className="relative grid overflow-hidden md:grid-cols-2">
        <div className="relative flex flex-col justify-center px-8 py-24 md:px-16">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#c9a468]">{clinic.specialty}</p>
          <h1
          ...
          <p className="mt-7 max-w-sm text-[#1a1a1a]/65">{clinic.welcomeMessage}</p>
          <div className="mt-9 flex flex-wrap gap-4">
            <a href={clinic.whatsapp} className="rounded-full bg-[#1a1a1a] px-7 py-3.5 text-sm font-medium text-[#f7f3ec] transition hover:bg-[#c9a468] hover:text-[#1a1a1a]">
              Agendar valoración
            </a>
            <a href={`tel:${clinic.phoneHref}`} className="rounded-full border border-[#1a1a1a]/25 px-7 py-3.5 text-sm font-medium transition hover:border-[#1a1a1a]/50">
              Llamar al consultorio
            </a>
          </div>
        </div>
        <div className="relative hidden min-h-[70vh] bg-[#1a1a1a] md:block">
          <div className="absolute left-1/2 top-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#c9a468]/20 blur-3xl" aria-hidden />
          <div className="absolute left-1/2 top-1/2 h-44 w-44 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#c9a468]/40" aria-hidden />
          <p
            className="absolute bottom-10 left-10 right-10 text-2xl text-[#f7f3ec]/90"
            style={{ fontFamily: "var(--f-playfair)" }}
          >
      ...
      <section id="especialista" className="grid gap-10 px-8 py-20 md:grid-cols-[1.4fr_1fr] md:px-16">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-[#c9a468]">Tu especialista</p>
          <h2 className="mt-3 text-4xl" style={{ fontFamily: "var(--f-playfair)" }}>{clinic.doctor}</h2>
          <p className="mt-5 max-w-md text-[#1a1a1a]/65">
            Médica cirujana egresada de la {clinic.school}, especializada en {clinic.specialty.toLowerCase()}.
          </p>
          <p className="mt-3 max-w-md text-sm text-[#1a1a1a]/50">Cédula profesional {clinic.license}</p>
        </div>
        <dl className="space-y-5 border-l border-[#c9a468]/40 pl-8">
          <div>
            <dt className="text-xs uppercase tracking-wide text-[#1a1a1a]/45">Años de práctica</dt>
            <dd className="text-2xl" style={{ fontFamily: "var(--f-playfair)" }}>{clinic.experienceYears}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-[#1a1a1a]/45">Pacientes atendidas</dt>
      ...
      <section id="servicios" className="border-t border-[#1a1a1a]/10 px-8 py-20 md:px-16">
        <h2 className="text-4xl" style={{ fontFamily: "var(--f-playfair)" }}>Tratamientos</h2>
        <div className="mt-12 divide-y divide-[#1a1a1a]/10">
          {services.map((s, i) => (
            <div key={s.name} className={`group relative flex flex-wrap items-baseline justify-between gap-4 py-6 ${s.isUrgency ? "bg-[#c9a468]/10 -mx-8 px-8 md:-mx-16 md:px-16" : ""}`}>
              <span className="absolute left-0 top-0 h-px w-0 bg-[#c9a468] transition-all duration-300 group-hover:w-full" aria-hidden />
              <div className="flex items-baseline gap-5">
                <span className="font-mono text-xs text-[#c9a468]">{String(i + 1).padStart(2, "0")}</span>
                <div>
                  <h3 className="text-lg" style={{ fontFamily: "var(--f-playfair)" }}>{s.name}</h3>
                  <p className="mt-1 text-sm text-[#1a1a1a]/55">{s.description}</p>
                </div>
              </div>
              <div className="text-right">
                <span className="text-lg font-medium">{s.price}</span>
                <span className="ml-2 text-xs uppercase tracking-wide text-[#1a1a1a]/40">{priceTypeLabel[s.priceType]}</span>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-8 flex flex-wrap gap-2">
          {paymentMethods.map((m) => (
            <span key={m} className="rounded-full border border-[#1a1a1a]/15 px-3 py-1 text-xs text-[#1a1a1a]/65">{m}</span>
          ))}
        </div>
      </section>

      {/* Ubicación */}
      <section id="ubicacion" className="grid gap-10 border-t border-[#1a1a1a]/10 px-8 py-20 md:grid-cols-2 md:px-16">
        <div>
          <h2 className="text-3xl" style={{ fontFamily: "var(--f-playfair)" }}>Ubicación</h2>
          <address className="mt-4 not-italic text-[#1a1a1a]/65">
            {clinic.address.street}<br />
            {clinic.address.neighborhood}<br />
            {clinic.address.zip}
          </address>
          <p className="mt-2 text-sm text-[#1a1a1a]/50">{clinic.address.reference}</p>
          <a href={clinic.address.mapsUrl} className="mt-3 inline-block text-sm font-medium text-[#c9a468] underline-offset-4 hover:underline">
            Ver en Google Maps →
          </a>
        </div>
        <div className="divide-y divide-[#1a1a1a]/10 self-start text-sm">
          <h2 className="pb-4 text-3xl" style={{ fontFamily: "var(--f-playfair)" }}>Horario</h2>
          {schedule.map((row) => (
            <div key={row.day} className="flex justify-between py-3">
              <span className="text-[#1a1a1a]/65">{row.day}</span>
              <span className={row.hours === "Cerrado" ? "text-[#1a1a1a]/35" : "font-medium"}>{row.hours}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="testimonios" className="border-t border-[#1a1a1a]/10 bg-[#1a1a1a] px-8 py-20 text-[#f7f3ec] md:px-16">
        <h2 className="text-3xl" style={{ fontFamily: "var(--f-playfair)" }}>Lo que dicen nuestras pacientes</h2>
        <div className="mt-12 grid gap-10 md:grid-cols-3">
          {testimonials.map((t) => (
            <figure key={t.name}>
              <span className="text-5xl text-[#c9a468]" style={{ fontFamily: "var(--f-playfair)" }}>&ldquo;</span>
              <blockquote className="mt-2 text-[#f7f3ec]/85">{t.quote}</blockquote>
              <figcaption className="mt-4 text-xs uppercase tracking-wide text-[#f7f3ec]/45">{t.name} · {t.treatment}</figcaption>
            </figure>
          ))}
        </div>
      </section>
