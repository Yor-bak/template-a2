...
import { ServiceForm } from "@/modules/especialista/components/ServiceForm";
...
export default function DashboardServiciosPage() {
  const { services, isLoading, loadError, toggleActive, deleteService, serviceHasAppointments } = useServices();
  ...
  async function handleToggle(service: Service) {
    try {
      ...
      setLastAction(`Error al cambiar estado de "${service.name}".`);
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      setLastAction(`Error al eliminar "${deletingService.name}".`);
    }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {isLoading && (
        <div className="mb-4 flex items-center gap-2 bg-sky-50 border border-sky-200 text-sky-700 text-sm px-4 py-3 rounded-xl">
      ...
      {loadError && (
        <div className="mb-4 flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-xl">
          ...
          {loadError} — mostrando datos locales.
        </div>
      )}
