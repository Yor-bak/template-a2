...
import { ChevronLeft, Phone, Mail, Calendar, ShieldAlert, Lock, Star, Plus, X } from "lucide-react";
...
export default function PatientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasHistory = hasFeature(config.plan, "clinical_history");
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[var(--color-muted-text)] text-sm">Paciente desde {formatShortDate(patient.firstVisitAt)}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          <div className="bg-white border border-[var(--color-border)] rounded-2xl p-5 shadow-sm md:col-span-2">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2 text-[var(--color-muted-text)]"><Mail className="w-4 h-4 text-[var(--color-accent)]" />{patient.email}</div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl p-4 text-sm text-amber-800">
            ...
            <div>
              ...
              <p className="text-xs">Este módulo contiene datos médicos privados del paciente. Solo visible para el dentista y personal autorizado. Nunca se comparte públicamente.</p>
            </div>
          </div>
          ...
          {!hasHistory ? (
            <div className="border border-dashed border-[var(--color-border)] rounded-2xl p-10 text-center">
              ...
              <h3 className="font-bold text-[var(--color-text)] mb-2 flex items-center justify-center gap-2">
                ...
                Historial clínico — Plan Premium
              </h3>
              <p className="text-sm text-[var(--color-muted-text)] max-w-sm mx-auto">
                Registra el expediente clínico completo, notas de cada sesión, antecedentes médicos y tratamientos realizados.
              </p>
            </div>
          ) : (
            <>
