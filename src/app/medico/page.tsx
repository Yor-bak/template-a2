...
export default function MedicoTemplate() {
  return (
    <div
      className={`${lora.variable} ${nunitoSans.variable} min-h-screen bg-[#f4f7f6] text-[#15324a]`}
      style={{ fontFamily: "var(--f-nunito-sans)" }}
    >
      {/* Header */}
      <header className="border-b border-[#15324a]/10 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-[#2563a8]">Consulta médica</p>
            <h1 className="text-lg font-semibold" style={{ fontFamily: "var(--f-lora)" }}>{doctor.name}</h1>
          </div>
          <a href={doctor.whatsapp} className="rounded-md bg-[#2563a8] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[#1d4e88]">
            Agendar consulta
          </a>
        </div>
        <Pulse className="h-3 w-full text-[#2563a8]/30" />
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-14">
        ...
        <section className="grid gap-10 pb-14 md:grid-cols-[1.3fr_1fr] md:items-center">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#2563a8]">{doctor.specialty} · CDMX</p>
            ...
            <p className="mt-5 max-w-lg text-[#15324a]/70">{doctor.welcomeMessage}</p>
            <div className="mt-8 flex flex-wrap gap-4">
              <a href={doctor.whatsapp} className="rounded-md bg-[#2563a8] px-6 py-3 text-sm font-medium text-white transition hover:bg-[#1d4e88]">
                Agendar consulta
              </a>
              <a href={`tel:${doctor.phoneHref}`} className="rounded-md border border-[#15324a]/20 px-6 py-3 text-sm font-medium transition hover:border-[#15324a]/50">
                Llamar al consultorio
              </a>
            </div>
          </div>
          <div className="rounded-2xl border border-[#15324a]/10 bg-white p-6">
            <p className="text-xs uppercase tracking-[0.2em] text-[#2563a8]">Signos vitales del consultorio</p>
            <dl className="mt-4 space-y-4">
              {vitals.map((v) => (
                <div key={v.label} className="flex items-baseline justify-between border-b border-[#15324a]/10 pb-3 last:border-0 last:pb-0">
                  <dt className="text-sm text-[#15324a]/60">{v.label}</dt>
                  <dd className="font-mono text-xl font-semibold text-[#2563a8]">{v.value}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <section id="servicios" className="py-14">
          ...
          <table className="mt-8 w-full border-collapse text-sm">
            <tbody>
              {services.map((s) => (
                <tr key={s.name} className={`border-b border-[#15324a]/10 ${s.isUrgency ? "bg-[#d6453d]/5" : ""}`}>
                  <td className="py-4 pr-6 align-top">
                    <div className="font-medium">{s.name}</div>
                    <div className="mt-1 text-[#15324a]/60">{s.description}</div>
                  </td>
                  <td className="py-4 text-right align-top whitespace-nowrap">
                    <div className="font-semibold text-[#2563a8]">{s.price}</div>
                    <div className="text-xs text-[#15324a]/50">{priceTypeLabel[s.priceType]}</div>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <section id="ubicacion" className="grid gap-10 py-14 md:grid-cols-2">
          <div>
            ...
            <address className="mt-4 not-italic text-[#15324a]/70">
              {doctor.address.street}<br />
              {doctor.address.neighborhood}<br />
              {doctor.address.zip}
            </address>
            <p className="mt-2 text-sm text-[#15324a]/60">{doctor.address.reference}</p>
            <a href={doctor.address.mapsUrl} className="mt-3 inline-block text-sm font-medium text-[#2563a8] underline-offset-4 hover:underline">
              Ver en Google Maps →
            </a>
            <div className="mt-8 flex flex-wrap gap-2">
              {paymentMethods.map((m) => (
                <span key={m} className="rounded-md border border-[#15324a]/15 px-3 py-1 text-xs text-[#15324a]/70">{m}</span>
              ))}
            </div>
          </div>
          <div className="divide-y divide-[#15324a]/10 self-start rounded-2xl border border-[#15324a]/10 bg-white p-6 text-sm">
            {schedule.map((row) => (
              <div key={row.day} className="flex justify-between py-2">
                <span className="text-[#15324a]/70">{row.day}</span>
                <span className={row.hours === "Cerrado" ? "text-[#15324a]/40" : "font-medium"}>{row.hours}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Urgencias */}
        <section id="urgencias" className="my-14 rounded-2xl bg-[#15324a] px-8 py-10 text-white">
          <p className="text-xs uppercase tracking-[0.2em] text-[#7fb1e0]">Urgencia ambulatoria</p>
          ...
          <div className="mt-6 flex gap-4">
            <a href={`tel:${doctor.phoneHref}`} className="rounded-md bg-[#d6453d] px-6 py-3 text-sm font-medium text-white">Llamar ahora</a>
        ...
        <section id="testimonios" className="py-14">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {testimonials.map((t) => (
              <figure key={t.name} className="rounded-2xl border border-[#15324a]/10 bg-white p-5">
                <blockquote className="text-[#15324a]/80">&ldquo;{t.quote}&rdquo;</blockquote>
                <figcaption className="mt-3 text-xs text-[#15324a]/50">{t.name} · {t.treatment}</figcaption>
              </figure>
            ))}
          </div>
        </section>
