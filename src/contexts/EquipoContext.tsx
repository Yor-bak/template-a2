...
function buildEmpty(): EquipoStore {
  return { workers: [], commissions: [], settlements: [], assignments: {}, tasks: [], invoices: [] };
}
...
function loadStore(): EquipoStore {
  try {
    ...
    // Merge with an empty store so older persisted stores (without tasks/invoices) stay valid.
    return { ...buildEmpty(), ...(JSON.parse(raw) as Partial<EquipoStore>) } as EquipoStore;
  } catch {
    return buildEmpty();
  }
}
