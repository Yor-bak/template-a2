...
import { PAYMENT_METHOD_LABEL, scheduleFromOpeningHours } from "@/lib/profileUtils";
...
const serviceIcons: ((p: Svg) => React.JSX.Element)[] = [
  IconStethoscope, IconSyringe, IconScalpel, IconBed, IconFlask, IconScan,
  IconTooth, IconDroplet, IconScissors, IconBowl, IconShield, IconClock,
];

const fallbackServices: { Icon: (p: Svg) => React.JSX.Element; title: string; desc: string; urgent?: boolean }[] = [
...
export function VeterinarioTemplate03({ profile, onPaletteChange, isPreview = false }: TemplateProps) {
  const {
    specialist,
    business,
  ...
  const activeProfileServices = profileServices.filter((s) => s.isActive);
  const services =
    activeProfileServices.length > 0
      ? activeProfileServices.map((s, i) => ({
          Icon: serviceIcons[i % serviceIcons.length],
          title: s.name,
          desc: s.shortDescription,
          urgent: s.isEmergency,
        }))
      : fallbackServices;
